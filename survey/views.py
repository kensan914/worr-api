...
class AccountDeleteSurveyAPIView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        account_delete_survey_serializer = AccountDeleteSurveySerializer(
            data=post_data)
        if account_delete_survey_serializer.is_valid():
            account_delete_survey_serializer.save()
            return Response(status=status.HTTP_201_CREATED)
        else:
            return Response(data=account_delete_survey_serializer.errors, status=status.HTTP_404_NOT_FOUND)
