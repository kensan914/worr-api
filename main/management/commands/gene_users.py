...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(user_N):
            ...
            user.save()
            user.features.set(
                Feature.objects.filter(key__in=self.get_random_params_keys(Feature.objects.all(), probability_num))
            )
            user.genre_of_worries.set(
                GenreOfWorries.objects.filter(key__in=self.get_random_params_keys(GenreOfWorries.objects.all(), probability_num))
            )
