...
class NotificationType(models.TextChoices):
    ...
    CANCEL_TALK_REQUEST_TO_RES = (
        "cancel_talk_request_to_res",
        "レスポンスユーザへのトークリクエストのキャンセル",
    )
    CANCEL_TALK_REQUEST_TO_REQ = (
        "cancel_talk_request_to_req",
        "リクエストユーザへのトークリクエストのキャンセル",
    )
...
class Notification(models.Model):
    ...
    subject = models.ForeignKey(
        ...
        null=True,
        on_delete=models.CASCADE,
        related_name="subject_notification",
    )
    type = models.CharField(
        verbose_name="タイプ",
        max_length=40,
        choices=NotificationType.choices,
        default=NotificationType.TALK_REQUEST,
    )
