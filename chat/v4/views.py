...
class RoomsAPIView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({
            ...
            'has_more': not(len(rooms) < self.paginate_by)
        }, status.HTTP_200_OK)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if room_serializer.is_valid():
            ...
            return Response(data=room_serializer.errors, status=status.HTTP_409_CONFLICT)
...
class RoomImagesAPIView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        room_serializer = RoomSerializer(
            instance=room, data=request_data, partial=True)
        if room_serializer.is_valid():
            room_serializer.save()
...
class RoomLeftMembersAPIView(views.APIView):
    ...
    def validate_member_id(cls, _member_id, _room):
        # roomのメンバーではない
        if not (str(_room.owner.id) == str(_member_id) or _room.participants.filter(id=_member_id).exists()):
            return Response(status=status.HTTP_403_FORBIDDEN)
        ...
        if not _room.is_end:
            ...
            if _room.participants.count() > 0:
                ChatConsumer.send_end_talk(
                    _room.id, RoomSerializer(_room).data)
        _room.save()
    ...
    def post(self, request, *args, **kwargs):
        ...
        # send 退室メッセ―ジ
        leave_message = request.data['leave_message'] if 'leave_message' in request.data else None
