...
class RoomSerializer(serializers.ModelSerializer):
    class Meta:
        model = RoomV4
        fields = ('id', 'name', 'image', 'image_src', 'owner', 'owner_id', 'participants', 'left_members',
                  'max_num_participants', 'is_exclude_different_gender', 'created_at', 'is_end', 'is_active')
        read_only_fields = ('id', 'created_at', 'is_end', 'is_active', 'image')

    owner = UserSerializer(read_only=True)
    owner_id = serializers.PrimaryKeyRelatedField(
        queryset=Account.objects.all(), write_only=True)  # post only
    image = serializers.SerializerMethodField(required=False)
    image_src = serializers.ImageField(
        source='image', write_only=True, required=False)  # post only
