...
class RoomV4Admin(admin.ModelAdmin):
    list_display = ('name', 'format_image', 'format_default_image', 'owner', 'format_participants', 'format_left_members',
                    'format_closed_members', 'is_exclude_different_gender', 'created_at', 'format_is_talking', 'is_active',)
    list_display_links = ('name',)
    search_fields = ('owner__username',)
    date_hierarchy = 'created_at'
    list_filter = ('is_end', 'is_active',)
    raw_id_fields = ('owner',)
    ...
    def format_image(self, obj):
        if obj.image:
            return format_html('<a href={} target="_blank"><img src="{}" width="100" style="border-radius: 8px" /></a>', obj.image.url, obj.image.large.url)
    ...
    def format_default_image(self, obj):
        if obj.default_image:
            return format_html('<a href={} target="_blank"><img src="{}" width="100" style="border-radius: 8px" /></a>', obj.default_image.image.url, obj.default_image.image.large.url)
...
class MessageV4Admin(admin.ModelAdmin):
    list_display = ('text', 'sender', 'room', 'time',
                    'format_stored_on_participants', 'format_read_participants')
...
class DefaultRoomImageAdmin(admin.ModelAdmin):
    list_display = ('format_image', 'file_name',)
